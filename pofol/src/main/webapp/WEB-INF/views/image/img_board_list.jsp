	...
	<div class="container">
		...
		<table class="table">
			...
			<c:forEach var="dto" items="${list}">
				<tr class="dataRow">
					<td class="bno">${dto.no}</td>
					<%-- <td class="title"><a href="<c:url value="/image/view?bno=${dto.bno}&inc=1"/>"><c:out value='${dto.title}' /></a> <span
						style="color: #FF0000">(${dto.comment_cnt})</span></td> --%>
					<td class="img"><img src="${dto.fileName}" width=120px height="80px" > </td>	
		...
		<ul class="pagination">
	        ...
	        <c:if test="${totalCnt!=null && totalCnt!=0}">
	          <c:if test="${ph.showPrev}">
	          	<li class="page-item">
	            <a class="page-link" href="<c:url value="/image/list.do${ph.sc.getQueryString(ph.beginPage-1)}"/>">&lt;</a>
	            <!-- <i class="glyphicon glyphicon-fast-backward"></i> -->
	            </li>
	          </c:if>
	          <c:forEach var="i" begin="${ph.beginPage}" end="${ph.endPage}">
	            <li class="page-item ${i==ph.sc.page? "active" : ""}"><a class="page-link" href="<c:url value="/image/list.do${ph.sc.getQueryString(i)}"/>">${i}</a></li>
	          </c:forEach>
	          <c:if test="${ph.showNext}">
	            <li class="page-item">
	            <a class="page-link" href="<c:url value="/image/list.do${ph.sc.getQueryString(ph.endPage+1)}"/>">&gt;</a>
	    ...
	    <br>
		<a class="btn btn-default" href="<c:url value="/image/write.do"/>">등록</a>
	</div>
</body>
</html>